...
void copy_matrices_dot(matrix3D<double>UU, matrix3D<double>VV, int dest, int source,
                       int imax, int jmax) {
    for (size_t i = 1; i <= imax - 1; ++i)
    {
        for (size_t j = 1; j <= jmax; ++j)
        {
            UU.at(dest).at(i).at(j) = UU.at(source).at(i).at(j);
        }
    ...
    for (int i = 1; i <= imax; ++i)
    {
        ...
        {
            VV.at(dest).at(i).at(j) = VV.at(source).at(i).at(j);
        }
...
void saveToFile(const std::vector<double> &arr, std::string fName, std::string problem, std::string method){

    std::ofstream file;
    file.open(("../Results/" + problem + "/" + method + "/" + fName));
